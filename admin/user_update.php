...
if($image_size<=10000000){
    ...
    $qq="UPDATE `user` SET `roll_id_fk`='$fk',`users_name`='$a',`user_email`='$b',`user_image`='$path',`user_passward`='$d' WHERE users_id=' $id'";
$ress=mysqli_query($con,$qq);

          

if($ress){
    echo"<script>alert('updated');window.location.href='user_table.php'</script>";
}
else{
    ...
    echo"<script>alert('image format');window.location.href='user_table.php'</script>";
        ...
        $qq="UPDATE `user` SET `roll_id_fk`='$fk',`users_name`='$a',`user_email`='$b',`user_passward`='$d' WHERE users_id=' $id'";
        $ress=mysqli_query($con,$qq);
              if($ress){
                  echo "<script>alert('updated');window.location.href='user_table.php'</script>";
              }
